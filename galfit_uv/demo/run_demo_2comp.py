#!/usr/bin/env python
"""
Demonstration of the galfit_uv package — two-component model.

Uses real ALMA data (GQC J0054-4955) to show:
  1. Exporting visibility data from a measurement set
  2. Building and fitting a Sersic(n=1) + point source model
     (exponential disk + unresolved nucleus) with fixed Sersic index
  3. Plotting data vs model with fit statistics
...
# ------------------------------------------------------------------ #
print("=" * 60)
print("2. Setting up Sersic(n=1) + Point source model")
print("=" * 60)
...
OUTPATH = str(DEMO_DIR / 'fit_output_2comp')
print()
...
fig = plot_uv(dvis, mvis, n_bins=15, scale='log', outpath=str(FIGS_DIR),
              fname='uvplot_2comp.png', show_im=False, show_log_x=True,
...
for fname in diag_files:
    ...
    dst = str(FIGS_DIR / fname.replace('.png', '_2comp.png'))
...
if os.path.exists(chains_src):
    shutil.copy2(chains_src, str(FIGS_DIR / 'chains_2comp.png'))
    print(f"  Copied chains_2comp.png")
...
clean = clean_image(MS_FILE, dvis.u, dvis.v, mvis, wle,
                    outdir=str(DEMO_DIR / 'clean_output_2comp'),
                    source_size=result.bestfit[labels.index('sersic:Re')],
...
fig = plot_clean_images(clean['data_image'], clean['model_image'],
                        ...
                        outpath=str(FIGS_DIR), fname='clean_images_2comp.png')
plt.close(fig)
